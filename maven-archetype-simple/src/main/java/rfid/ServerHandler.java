...
public class ServerHandler {
    ...
    private JSONHandler configs;
    private BusCSVHandler bus_handler;
    ...
    class ServerOffline implements Runnable {
        ...
        @Override
        public void run() {
            BusCSVHandler.BusHolder next = bus_handler.getOfflineBusHolderNext();
            while (next != null) {
                // if post fail, add it to offline_bus_holder. Wait for 1 min for server to reestablish
                if (!pushServer(next.getBus())) {
                    bus_handler.updateOfflineBusHolder(next);
                    try {
                        Thread.sleep(Long.parseLong(configs.getField(JSONField.INTERNET_CONNECT_TIME_OUT_MS)));
                ...
                next = bus_handler.getOfflineBusHolderNext();
            }
        }
    ...
    public static ServerHandler getServerHandler() {
        ...
        server_handler.configs = JSONHandler.getJSONHandler(FileDir.CONFIGS_DIR).getJSONObjectFromFileConfig();
    ...
    public ServerHandler setBusHandler(BusCSVHandler bus_handler){
        this.bus_handler = bus_handler;
    ...
    public void prepareData(String RFID_input, String MAC_address) {
        ...
        bus_handler.updateBusContent(bus_info);
        ...
        if(!hasInternetConectivity()){
            ...
            bus_handler.updateOfflineBusHolder(bus_handler.new BusHolder(bus_info, bus_handler.getCurrLine()));
            return;
        }
    ...
    public boolean hasInternetConectivity() {
        try {
            URL url = new URL(configs.getField(JSONField.URL));
    ...
    private boolean pushServer(Bus bus_info){
        ...
        HttpRequest request = HttpRequest.newBuilder().uri(URI.create(this.configs.getField(JSONField.URL)))
