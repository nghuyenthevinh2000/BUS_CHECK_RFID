...
public class BusCSVHandler {
    ...
    public synchronized void saveOfflineBusHolder(){
        ...
        states.saveArrayToFile(lines);
    }

    // update bus content
    public void updateBusContent(Bus bus_info) {
        OutputStreamWriter history_csv_writer = FileHandler.getWriter(FileDir.BUS_HISTORY, true);
        StatefulBeanToCsv<Bus> history_bus_writer = new StatefulBeanToCsvBuilder<Bus>(history_csv_writer).build();
        ...
        try{
            ...
            LogHandler.log_err(BusCSVHandler.class.getName(), (Throwable) e);
        }
    }
    ...
    private void initializeBusHistory(){
        ...
        states.saveArrayToFile(new String[0]);
    }
}
