...
public class PortInputHandler extends InputHandler {
   ...
   public static PortInputHandler getInputHandler() {
      ...
      while (true) {
         ...
         try {
            Thread.sleep(
                  Long.parseLong(inputHandler.config_handler.getField(JSONField.DEVICE_CONNECT_FAIL_TIME_OUT_MS)));
   ...
   public String getRFIDInput() {
      ...
      comPort.addDataListener(new SerialPortDataListener() {
         ...
         public void serialEvent(SerialPortEvent event) {
            ...
            if (writer != null && availableByte > 0) {
               try {
                  writer.append(String.format("Total bytes = %s, Bytes failed to read = %s, ", availableByte, availableByte - readByte));
               } catch (IOException e) {
                  e.printStackTrace();
               }
            }else{
