...
public class ServerHandler {
    ...
    public boolean canPostServer(String RFID_INPUT){
        boolean canPostServer = false;
        if(bus_rfid_handler.hasRFID_INPUT(RFID_INPUT))
            canPostServer = true;

        return canPostServer;
    }
    ...
    public void postServer(String RFID_input, String MAC_address) {
        ...
        String log_content = String.format("DATE_TIME = %s, RFID_input = %s, MAC_address = %s %n",
                bus_info.getDATE_TIME(), RFID_input, MAC_address);
        if (log_writer != null) {
            try {
    ...
    public static boolean postServerNoti(String json_body) {
        // MEGUMIN: nghĩ ra cách tốt hơn để giải quyết đống mess ở postServer. Thiết kế
        // lại phần này
        return false;
    }
    ...
    private boolean hasInternetConectivity() {
        ...
        try {
            InetAddress[] addresses = InetAddress.getAllByName(config_handler.getField(JSONField.URL));
            for (InetAddress address : addresses) {
                if (address.isReachable(Integer.parseInt(config_handler.getField(JSONField.INTERNET_CONNECT_TIME_OUT_MS))))
                    hasInternet = true;
            }
        } catch (IOException e) {
            e.printStackTrace();
