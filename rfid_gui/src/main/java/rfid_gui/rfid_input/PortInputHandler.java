...
import rfid_gui.JSONHandler;
import rfid_gui.LogHandler;
import rfid_gui.MainProgram;
import rfid_gui.FileHandler.FileDir;
import rfid_gui.JSONHandler.JSONField;
...
public class PortInputHandler extends InputHandler {
   ...
   public static PortInputHandler getInputHandler() {
      ...
      if (inputHandler.comPortList.length == 0) {
         ...
         MainProgram.logPopUp("Fail to connect device");
         return null;
      }
      inputHandler.configs = JSONHandler.getJSONHandler(FileDir.CONFIGS_DIR).getJSONObjectFromFileConfig();

      inputHandler.comPort = inputHandler.comPortList[0];
      inputHandler.setPortConfig();
      inputHandler.closePort();
      inputHandler.openPort();
      return inputHandler;
   }
   ...
   public String getRFIDInput() {
      ...
      if (!comPort.isOpen()) {
         ...
         MainProgram.logPopUp("Port is occupied, now exit");
         return null;
      }
   ...
   private synchronized byte[] getOutput() {
      ...
      while (this.output == null) {
         try {
            wait(Long.parseLong(configs.getField(JSONField.READ_DATA_TIME_OUT_MS)));
            if(this.output == null) 
               return new_output;
